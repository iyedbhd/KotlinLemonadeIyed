...
class MainActivity : AppCompatActivity() {
    ...
    private val lemonStateMap = mapOf(
        SELECT to mapOf(
            "img" to R.drawable.lemon_tree,
            "txt" to R.string.lemon_select
        ),
        SQUEEZE to mapOf(
            "img" to R.drawable.lemon_squeeze,
            "txt" to R.string.lemon_squeeze
        ),
        DRINK to mapOf(
            "img" to R.drawable.lemon_drink,
            "txt" to R.string.lemon_drink
        ),
        RESTART to mapOf(
            "img" to R.drawable.lemon_restart,
            "txt" to R.string.lemon_empty_glass
        )
    )
    ...
    private fun setViewElements() {
        ...
        lemonStateMap[lemonadeState]?.let { it["img"]?.let { it1 -> lemonImage?.setImageResource(it1) } };
        lemonStateMap[lemonadeState]?.let { it["txt"]?.let { it1 -> textAction.text=resources.getString(it1) } };
